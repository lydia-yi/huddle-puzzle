...
func move(direction: Vector2):
	...
	elif target_tile == tile_map.local_to_map(penguin_b.global_position):
		return 
...
func check_freezing():
	if Global.game_over == false:
		...
		var B_coords : Vector2 = tile_map.local_to_map(penguin_b.global_position)
		...
		for n in neighbor_offsets:
			...
			if B_coords == neighbor_coords: 
				freezing = false
				return
			elif A_coords == neighbor_coords:
				freezing = false
				return
			else:
				freezing = true
